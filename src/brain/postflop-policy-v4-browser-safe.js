...
export function postflopPolicyV4Decision(state, context = {}) {
  ...
  const made=eval7([...hero,...board]),draw=drawInfo(hero,board),pc=pairClass(hero,board),pot=n(state?.pot),call=n(state?.toCall),st=board.length>=5?'river':board.length===4?'turn':'flop';
  if(legal.has('CHECK')&&!legal.has('CALL')){
    let pref=['CHECK'];
    if(legal.has('BET')&&(made.cat>=2||(made.cat===1&&(pc==='top-pair'||pc==='overpair'))||(st!=='river'&&(draw.flushDraw||draw.straightDraw))))pref=['BET','CHECK'];
  ...
  if(made.cat>=3){pref=legal.has('RAISE')&&frac<=.75?['RAISE','CALL']:['CALL'];why=`${made.name} é mão forte para continuar`;}
    ...
    if(pc==='overpair'||pc==='top-pair'){
      ...
      const hasDraw=draw.flushDraw||draw.straightDraw;const lim=st==='flop'?(hasDraw?.60:.48):st==='turn'?(hasDraw?.45:.32):.20;pref=frac<=lim?['CALL']:['FOLD'];why=`${pc}; bluff-catcher fraco, decisão guiada pelo preço`;
    ...
    const lim=(draw.flushDraw&&draw.straightDraw)?.90:.65;pref=frac<=lim?['CALL']:['FOLD'];why=`draw ${draw.flushDraw?'de flush ':''}${draw.straightDraw?'de sequência':''}; preço comparado às odds`;
  }
