...
export function reasoningFingerprint({ handId, state, events = [], actorName = null, opponentStats = null } = {}) {
  ...
  return [handId || 0, state?.street || '-', hero, board, Number.isFinite(state?.pot) ? Math.round(state.pot) : '-', actions, actorName || '-', history, statsKey].join('|').slice(0, 512);
}
...
export function buildReasoningPayload({ handId, state, events = [], actorName = null, potBefore = null, opponentStats = null, baseline = null } = {}) {
  const fingerprint = reasoningFingerprint({ handId, state, events, actorName, opponentStats });
  return {
    mode: 'replay',
    handId,
    fingerprint,
