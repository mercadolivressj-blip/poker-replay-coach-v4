...
function upscale(source,scale=3){
  const c=document.createElement('canvas');c.width=Math.max(80,Math.round(source.width*scale));c.height=Math.max(36,Math.round(source.height*scale));
...
function contrast(source){
  const c=upscale(source,3),ctx=c.getContext('2d',{willReadFrequently:true}),im=ctx.getImageData(0,0,c.width,c.height);
  for(let i=0;i<im.data.length;i+=4){const L=.299*im.data[i]+.587*im.data[i+1]+.114*im.data[i+2];const v=L>138?255:0;im.data[i]=v;im.data[i+1]=v;im.data[i+2]=v;}
...
export class ActionTextOcr{
  ...
  async read(source,{timeoutMs=1200}={}){
    const worker=await getWorker();if(!worker)return {text:'',confidence:0,parsed:null,unavailable:true};
    while(this.busy)await sleep(10);this.busy=true;
    try{
      const variants=[upscale(source,3),contrast(source)];let best={text:'',confidence:0,parsed:null};
      for(const image of variants){
        const t0=performance.now();
        const out=await Promise.race([worker.recognize(image),sleep(timeoutMs).then(()=>null)]);
        if(!out)continue;const text=(out.data?.text||'').replace(/\s+/g,' ').trim();const confidence=Number(out.data?.confidence)||0;const parsed=parsePokerStarsActionText(text);
