...
import { parseSeatText, parsePokerStarsNumber, seatRectsFromFelt, inferActionFromStacks, stableNumericObservation } from '../src/vision/pokerstars-table-reader-r14.js';
...
const rects = seatRectsFromFelt({ x: 0.20, y: 0.20, w: 0.60, h: 0.45 });
assert.equal(rects.length, 6);
assert(rects.every((r) => r.x >= 0 && r.y >= 0 && r.x + r.w <= 1.00001 && r.y + r.h <= 1.00001));
...
console.log(`VISUAL TABLE R14 passed · K6 equity ${(multi.equity * 100).toFixed(1)}%`);
