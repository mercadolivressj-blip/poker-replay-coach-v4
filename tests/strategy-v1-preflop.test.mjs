...
assert(bbFacingUnknown.decision);
assert.match(bbFacingUnknown.engine,/PREFLOP UNIVERSAL/);
assert(['DESISTIR','PAGAR','AUMENTAR'].includes(bbFacingUnknown.decision));
...
assert(bbBadLegacyHistory.decision);
assert.match(bbBadLegacyHistory.engine,/PREFLOP UNIVERSAL/);
...
assert.equal(coFacingCost.decision,'DESISTIR');
assert.match(coFacingCost.engine,/PREFLOP UNIVERSAL/);
...
assert(structuredVsOpen.decision);
...
assert.match(bbLimpOption44.engine,/PREFLOP UNIVERSAL/);
...
assert.match(bbLimpOptionAQ.engine,/PREFLOP UNIVERSAL/);

// Product requirement: cash preflop must not stay silent merely because position/node/depth are incomplete.
const unknownPosition=preflopDecision({
  heroCards:['Js','8c'],board:[],heroPosition:null,legalActions:['FOLD','CALL','RAISE'],actionHistory:[],
  heroStack:'2.00',effectiveStack:'2.00',blinds:'0.01/0.02',toCall:'0.06'
},{format:'cash',tableSize:'6max',handId:'unknown-position'});
assert.equal(unknownPosition.decision,'DESISTIR');
assert.match(unknownPosition.engine,/PREFLOP UNIVERSAL/);

const fortyFiveBbOpen=preflopDecision({
  heroCards:['Ah','8h'],board:[],heroPosition:'BTN',legalActions:['FOLD','RAISE'],actionHistory:['UTG FOLD','HJ FOLD','CO FOLD'],
  heroStack:'11.25',effectiveStack:'11.25',blinds:'0.10/0.25',toCall:null
},{format:'cash',tableSize:'6max',handId:'45bb-open'});
assert.equal(fortyFiveBbOpen.decision,'AUMENTAR');
assert.match(fortyFiveBbOpen.engine,/PREFLOP UNIVERSAL/);
assert.equal(fortyFiveBbOpen.chartCertified,false);

const twentyBbPremium=preflopDecision({
  heroCards:['As','Kh'],board:[],heroPosition:null,legalActions:['FOLD','CALL','RAISE'],actionHistory:[],
  heroStack:'5.00',effectiveStack:'5.00',blinds:'0.10/0.25',toCall:'0.50'
},{format:'cash',tableSize:'6max',handId:'20bb-premium'});
assert(['PAGAR','AUMENTAR','ALL-IN'].includes(twentyBbPremium.decision));
assert.match(twentyBbPremium.engine,/PREFLOP UNIVERSAL/);

console.log('strategy-v1-preflop parity + exact baseline + universal cash fallback regression: OK');
