...
export function decideFromRegisteredPack(raw,{mode='cEV',villainPosition='*',decisionKey='',minimumCertification='solver-verified',allowUnverified=false}={}){
 ...
 if(!gate.ready)return{version:'mtt-decision-kernel-v0.5',decision:null,status:'STATE_INCOMPLETE',gate,state};
 const resolvedVillain=villainPosition==='*'?(state.preflop.lastAggressorPosition||'*'):villainPosition;
 ...
 if(!hit?.distribution)return{version:'mtt-decision-kernel-v0.5',decision:null,status:'OUT_OF_COVERAGE',state,pack:hit?.pack??null};
 const certification=decisionCertificationGate(hit.pack,{minimum:minimumCertification,allowUnverified});
 if(!certification.allowed)return{version:'mtt-decision-kernel-v0.5',decision:null,status:'STRATEGY_NOT_CERTIFIED',state,pack:hit.pack,distribution:hit.distribution,certification};
 ...
 if(!choice)return{version:'mtt-decision-kernel-v0.5',decision:null,status:'NO_LEGAL_ACTION_IN_DISTRIBUTION',state,pack:hit.pack,distribution:hit.distribution,certification};
 return{version:'mtt-decision-kernel-v0.5',decision:choice.action,status:'DECISION',state,pack:hit.pack,distribution:choice.masked,seed,certification,depth:{effectiveBB:state.bb.effective,anchorBB:hit.pack.stackDepthBB,policy:hit.pack.depthPolicy||'exact'},context:{policy:hit.pack.contextPolicy||'generic',preflopContext:preflopContextFromState(state),villainPosition:resolvedVillain}};
}
