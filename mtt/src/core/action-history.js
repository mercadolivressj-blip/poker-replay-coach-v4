...
export function normalizeAction(a){
  ...
  if(typeof a==='string'){
    ...
    return{actor:p.length>1?p[0]:null,action:MAP[raw]||raw,amount:null,street:null};
  }
...
export function preflopPressure(history=[]){
  ...
  return{raiseCount:raises.length,callCount:calls.length,lastAggressor:raises.length?raises.at(-1).actor:null,node:raises.length>=3?'vs_4bet_plus':raises.length===2?'vs_3bet':raises.length===1?(calls.length?'vs_open_multiway':'vs_open'):'unopened'};
}
