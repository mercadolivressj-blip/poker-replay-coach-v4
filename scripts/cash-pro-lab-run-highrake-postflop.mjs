...
const manifestDir=path.dirname(manifestPath),solverPath=path.resolve(solverArg);
const summary={
  version:'cash-pro-lab-highrake-run-summary-v1',campaignId:manifest.campaignId,manifestPath,solverPath,
...
for(const [i,row] of jobs.entries()){
  const result=await executeHighRakeJob({manifestDir,row,solverPath,dryRun,resume});
...
summary.note='Strategy-oracle validation is not study certification. Alternative-EV authority and required independent-teacher consensus remain separate gates.';
